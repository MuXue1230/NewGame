...
class ServerNetworkController:
    ...
    def start(self) -> None:
        ...
        self.logger.info("Server socket binded (on %s %s)." %(self.HOST, self.PORT))
        self.socket.listen()
    ...
    def send(self, conn_id:int, data:Any) -> None:
        if conn_id < len(self.clients):
            ...
            if check == "READY":
                ...
                raise NetworkConnectionSendDataFailed("Server socket failed to send data to %s." %self.clients[conn_id], 
            ...
            raise NetworkConnectionIndexOutOfRange("The provided connection id (%d) is out of range." %conn_id, 
                                                    _from="cn.snowskystudio.newgame.network.ServerNetworkController - Line 43")
