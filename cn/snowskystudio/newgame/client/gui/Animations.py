...
class Animations:
    ...
    def __main_enter_animation_ready(self):
        ...
        for i in range(0, 256, 8):
            f = frame.copy()
            f.set_alpha(i)
            self.transition_animation.append(f)

        time.sleep(10)  # To wait for loadscreen initialize.

    @staticmethod
    def __main_enter_animation_tick(self):
        if self.animation_time < 256 // 8:
            self.screen.screen.get_screen().blit(self.transition_animation[self.animation_time], (0, 0))
            self.animation_time += 1
            return True
        self.screen.screen.get_screen().blit(self.transition_animation[self.animation_time - 1], (0, 0))
        return False

    @staticmethod
    def __main_out_animation_ready(self):
        self.screen.size = (self.screen.size[0] // 2, self.screen.size[1] // 2)
    ...
    @staticmethod
    def __main_out_animation_tick(self):
        if self.animation_time < 256 // 8:
            self.screen.screen.get_screen().blit(self.transition_animation[self.animation_time], (0, 0))
            self.animation_time += 1
            return True
    ...
    def __process_logo_animation_tick(self):
        if self.animation_time < 63 and self.describe:
            self.animation_time += 1
            self.screen.screen.get_screen().blit(self.logos[32], (
                (self.screen.config.get_screen().get_size()[0] - self.logos[32].get_width()) / 2, (
                        self.screen.config.get_screen().get_size()[1] - self.logos[
                    32].get_height()) / 2 - self.screen.c25))
            self.screen.screen.get_screen().blit(self.describes[63 - self.animation_time], (
                (self.screen.config.get_screen().get_size()[0] - self.logos[32].get_width()) / 2 + self.screen.c100, (
                        self.screen.config.get_screen().get_size()[1] - self.logos[
                    32].get_height()) / 2 + self.screen.c25))
            return True
        ...
        if self.animation_time < 33:
            self.animation_time += 1
            trans_x = ((self.screen.config.get_screen().get_size()[1] - self.logos[
                48 - 15 - self.animation_time].get_height()) / 2 - self.screen.c25 - self.screen.c25) * self.animation_time // (
                              48 - 15)
            trans_y = ((self.screen.config.get_screen().get_size()[0] - self.logos[
                48 - 15 - self.animation_time].get_width()) / 2 - self.screen.c50) * self.animation_time // (
                              48 - 15)
            self.screen.screen.get_screen().blit(self.logos[48 - 15 - self.animation_time], ((
                                                                                                     self.screen.config.get_screen().get_size()[
                                                                                                         0] -
                                                                                                     self.logos[
                                                                                                         48 - 15 - self.animation_time].get_width()) / 2 - trans_y,
                                                                                             (
                                                                                                     self.screen.config.get_screen().get_size()[
                                                                                                         1] -
                                                                                                     self.logos[
                                                                                                         48 - 15 - self.animation_time].get_height()) / 2 - self.screen.c25 - trans_x))
            return True
        self.screen.screen.get_screen().blit(self.logos[0], (self.screen.c50, self.screen.c25))
        return False
    ...
    def __process_loading_animation_tick(self):
        ...
        self.screen.screen.get_screen().blit(self.circle, (
            ...
            self.screen.config.get_screen().get_size()[
                1] - self.circle.get_height() / 2 - self.screen.c100))
        self.screen.screen.get_screen().blit(self.screen.text_load, (
            ...
            self.screen.config.get_screen().get_size()[
                1] - self.screen.text_load.get_height() / 2 - self.screen.c100))
    ...
    MAIN_OUT_ANIMATION = Animation(AnimationLocation('newgame', 'main/out'), logger, __main_out_animation_ready,
                                   __main_out_animation_tick)
